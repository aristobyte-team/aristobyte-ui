...
export const AristoByte = ({ size: size = 24 }: IconPropsType) => {
  ...
  const svgId = `${uid}-icon-aristobyte`;
  const primaryGradientId = `${uid}-aristobyte-primary-gradient`;
  const secondaryGradientId = `${uid}-aristobyte-secondary-gradient`;
  const tertiaryGradientId = `${uid}-aristobyte-tertiary-gradient`;
  const quaternaryGradientId = `${uid}-aristobyte-quaternary-gradient`;
  const quinaryGradientId = `${uid}-aristobyte-quinary-gradient`;
  const senaryGradientId = `${uid}-aristobyte-senary-gradient`;
  const luminosityFilterId = `${uid}-aristobyte-luminosity-filter`;
  const logoMaskId = `${uid}-aristobyte-logo-mask`;
  ...
  >
    <defs>
      ...
      <mask id={logoMaskId} x="-2.12" y="-2.12" width="1110.24" height="1478.88" maskUnits="userSpaceOnUse">
        <g filter={`url(#${luminosityFilterId})`}>
          <image
          //...
          />
        </g>
