...
export const RadioGroup: React.FC<IRadioGroup> = ({
  ...
  const radios = React.Children.toArray(children).filter(
    (child): child is React.ReactElement<IRadio> => React.isValidElement(child) && child.type === Radio
  );
  ...
  return (
    <div key={uniqueId} className={`radio-group ${`radio-group--${align}`} ${className}`} style={style}>
      {radios.map(({ props }) => (
        <Radio
          {...props}
