...
import { omit, kebabCase, mapKeys } from "@aristobyte-ui/utils";
    ...
    for (const [colorName, colorValue] of Object.entries(flatColors)) {
      ...
      try {
        ...
        resolved.utilities[cssSelector]![herouiColorVariable] =
          `${h} ${s}% ${l}%`;
        baseSelector &&
          (resolved.baseStyles[baseSelector]![herouiColorVariable] =
            `${h} ${s}% ${l}%`);
    ...
    for (const [key, value] of Object.entries(flatLayout)) {
      ...
      if (typeof value === "object") {
        for (const [nestedKey, nestedValue] of Object.entries(value)) {
          ...
          baseSelector &&
            (resolved.baseStyles[baseSelector]![nestedLayoutVariable] =
              nestedValue);
        }
      } else {
        ...
        baseSelector &&
          (resolved.baseStyles[baseSelector]![layoutVariablePrefix] =
            formattedValue);
      }
    }
  }
      ...
      theme: {
        extend: {
          ...
          backgroundImage: {
            ...
            "stripe-gradient-primary": createStripeGradient(
              "primary-200",
              "primary"
            ),
            ...
            "stripe-gradient-success": createStripeGradient(
              "success-200",
              "success"
            ),
            "stripe-gradient-warning": createStripeGradient(
              "warning-200",
              "warning"
            ),
            "stripe-gradient-danger": createStripeGradient(
              "danger-200",
              "danger"
            ),
          },
