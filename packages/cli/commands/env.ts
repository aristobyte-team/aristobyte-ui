...
export async function env(options: OptionValues) {
  const s = spinner();
  try {
...
${color.gray("|")}`
    ...
    const installed =
      options.packages || options.all || Object.keys(options).length === 0
...
${color.gray("|")}   ${color.white("Version:")} ${color.green(
        checkVersion(
          pm.packageManager,
          pm.version,
          MIN_VERSIONS[pm.packageManager]
        )
      )}`);
    }
...
${color.green("◇")}  ${color.cyan("Installed AristoByteUI Packages:")}`
      ...
      if (!installed.length) {
        ...
        installed.forEach((p) =>
          console.log(
            `${color.gray("|")}   ${color.green("-")} ${color.white(p)}`
          )
        );
      }
    }
