...
export async function doctor(options: OptionValues) {
  const s = spinner();
  try {
    s.start("Running doctor diagnostics…");

    console.log(
      `${color.green("◇")}  ${color.cyan("Doctor Diagnostics")}\n${color.gray("|")}`
    );
    ...
    if (options.configs || options.all || Object.keys(options).length === 0) {
      ...
      if (!configs.length) {
        console.log(
          `${color.gray("|")}   ${color.red("× No config files detected")}`
        );
      } else {
        configs.forEach((cfg) =>
          console.log(
            `${color.gray("|")}   ${color.green("✔")} ${color.white(cfg)}`
          )
        );
      }
    }
    ...
    if (
      (options.network || options.all || Object.keys(options).length === 0) &&
